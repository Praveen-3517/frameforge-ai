...
int** createPascalJagged(int n) {
        int** jagged = (int*)malloc((n) * sizeof(int))[];

        for (int r = 0; r < n; r++) {
            jagged[r] = (int*)malloc((r + 1) * sizeof(int)); // Row r has length r + 1
            for (int c = 0; c <= r; c++) {
                if (c == 0 || c == r) {
                    jagged[r][c] = 1;
                } else {
    ...
    void printJaggedArray(int** arr) {
        printf("Outer Array Length: %d (Rows)\n", n);
        for (int r = 0; r < n; r++) {
            printf("Row %d (length %d): ", r, arr[r].length);
    ...
    int main(void) {
        printf("=== Java Jagged Arrays (Heap Layout & Dynamic Row Lengths) ===\n");

        int** jaggedPascal = createPascalJagged(5);
        printJaggedArray(jaggedPascal);

        printf("\n--- Explicit Custom Uneven Allocation ---\n");
        int** uneven = (int*)malloc((4) * sizeof(int))[];
        uneven[0] = (int[]){ 10, 20 };
        uneven[1] = (int[]){ 30, 40, 50, 60, 70 };
        uneven[2] = (int[]){ 80 };
        uneven[3] = (int[]){ 90, 100, 110 };
