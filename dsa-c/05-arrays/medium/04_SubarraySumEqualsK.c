...
int subarraySum(int* nums, int k) {
        ...
        Map<int, int> prefixCountMap = new HashMap<>();
    ...
    int main(void) {
        printf("=== LeetCode 560: Subarray Sum Equals K (Prefix Sum + Map) ===\n");
        ...
        for (int i = 0; i < n; i++) {
            ...
            printf("Array: %-25s | k = %d ==> Count: %d\n",
                    java.util."[array]", kValues[i], count);
        }
        return 0;
}
