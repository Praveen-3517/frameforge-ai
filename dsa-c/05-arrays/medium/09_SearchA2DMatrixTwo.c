...
bool searchMatrix(int** matrix, int target) {
        if (matrix == NULL || n == 0 || matrix[0].length == 0) {
            return false;
        }
    ...
    int main(void) {
        printf("=== LeetCode 240: Search a 2D Matrix II (Stepwise O(M+N)) ===\n");
        ...
        for (size_t _idx = 0; _idx < sizeof(searchTargets)/sizeof(searchTargets[0]); _idx++) {
        int t = searchTargets[_idx];
            bool found = searchMatrix(matrix, t);
            printf("Target %2d ==> Found: %s\n", t, found);
        }
        return 0;
}
