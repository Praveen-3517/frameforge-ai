...
void printCollatzTrajectory(long long n) {
        if (n <= 0) {
...
"); return 0;
        ...
        printf("Start: %7d | Steps: %4d | Peak: %11d | Path: %s\n",
                n, steps, peak, preview);
    }

    int main(void) {
        printf("=== Collatz Conjecture / Hailstone Sequence Simulation ===\n");

        long long* testStarters = { 1, 6, 11, 27, 837799L };

        for (size_t _idx = 0; _idx < sizeof(testStarters)/sizeof(testStarters[0]); _idx++) {
        long long start = testStarters[_idx];
