...
static class Box {
    ...
    void swapPrimitives(int x, int y) {
        int temp = x;
        x = y;
        y = temp;
    }
    ...
    void swapReferences(Box b1, Box b2) {
        Box temp = b1;
        b1 = b2;
        b2 = temp;
    }
    ...
    void mutateState(Box b) {
        b.value = 999;
    }

    int main(void) {
        printf("=== Proof: Java is Strictly Pass-by-Value ===\n");

        // Test 1: Primitives
        int a = 10, b = 20;
        printf("1. Primitives Before Swap: a = %d, b = %d\n", a, b);
        swapPrimitives(a, b);
        printf("   Primitives After Swap : a = " + a + ", b = " + b + " (Unchanged!)\n\n");

        // Test 2: Reference Reassignment
        Box box1 = new Box(100);
        Box box2 = new Box(200);
        printf("2. Boxes Before Reference Swap: box1=%d, box2=%d\n", box1.value, box2.value);
        swapReferences(box1, box2);
        printf("   Boxes After Reference Swap : box1=" + box1.value + ", box2=" + box2.value + " (Unchanged!)\n\n");

        // Test 3: State Mutation
        Box box3 = new Box(50);
        printf("3. Box Before Mutation: value = %d\n", box3.value);
        mutateState(box3);
        printf("   Box After Mutation : value = " + box3.value + " (Mutated via shared heap reference!)\n");
        return 0;
}
