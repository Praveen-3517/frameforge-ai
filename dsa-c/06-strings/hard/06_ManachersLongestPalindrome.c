...
const char* longestPalindromeManacher(const char* s) {
        ...
        int n = n;
        int* P = (int*)malloc((n) * sizeof(int)); // Palindrome radius array
    ...
    int main(void) {
        printf("=== Manacher's Algorithm: O(N) Longest Palindromic Substring ===\n");
        ...
        for (const char* str : testStrings) {
            const char* lps = longestPalindromeManacher(str);
