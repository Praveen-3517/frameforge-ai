...
int findMaximizedCapital(int k, int w, int* profits, int* capital) {
        ...
        for (int i = 0; i < n; i++) {
            minCapitalHeap.offer((int[]){capital[i], profits[i]});
        }

        // Max-heap ordered by profit
        PriorityQueue<int> maxProfitHeap = new PriorityQueue<>(Collections.reverseOrder());
    ...
    int main(void) {
        ...
        int* profits1 = {1, 2, 3};
        int* capital1 = {0, 1, 1};
        printf("Max Capital 1: " + findMaximizedCapital(k1, w1, profits1, capital1) + " (Expected: 4)\n");

        int k2 = 3, w2 = 0;
        int* profits2 = {1, 2, 3};
        int* capital2 = {0, 1, 2};
        printf("Max Capital 2: " + findMaximizedCapital(k2, w2, profits2, capital2) + " (Expected: 6)\n");
        return 0;
}
