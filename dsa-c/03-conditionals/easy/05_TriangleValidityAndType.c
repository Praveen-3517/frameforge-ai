...
const char* analyzeTriangle(double a, double b, double c) {
        if (a <= 0 || b <= 0 || c <= 0) {
        ...
        double pythDiff = abs((sides[0] * sides[0] + sides[1] * sides[1]) - (sides[2] * sides[2]));
        if (pythDiff < 1e-9) {
    ...
    int main(void) {
        printf("=== Triangle Inequality & Geometric Classification ===\n");
