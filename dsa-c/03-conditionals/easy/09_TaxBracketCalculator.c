...
double computeTax(double income) {
    ...
    int main(void) {
        printf("=== Progressive Marginal Tax Bracket Calculator ===\n");

        double* testIncomes = { 8000, 10000, 25000, 50000, 75000, 100000, 150000, 300000 };

        for (size_t _idx = 0; _idx < sizeof(testIncomes)/sizeof(testIncomes[0]); _idx++) {
        double inc = testIncomes[_idx];
            double tax = computeTax(inc);
            double effectiveRate = (inc > 0) ? (tax / inc * 100.0) : 0.0;
            printf("Income: $%8.2f ==> Total Tax: $%8.2f | Effective Rate: %5.2f%%\n",
