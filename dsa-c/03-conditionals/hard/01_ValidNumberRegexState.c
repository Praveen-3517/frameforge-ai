...
// DFA Transition Table: transitionTable[state][charGroup]
    ...
    static int getCharGroup(char ch) {
    ...
    int main(void) {
        printf("=== LeetCode 65: Valid Number (Deterministic Finite Automaton) ===\n");

        const char*[] validExamples = {
            "2", "0089", "-0.1", "+3.14", "4.", "-.9", "2e10", "-90E3", "3e+7", "+6e-1", "53.5e93", "-123.456e789"
        };

        const char*[] invalidExamples = {
            "abc", "1a", "1e", "e3", "99e2.5", "--6", "-+3", "95a54e53", ".", ".-4"
        };

        printf("--- Valid Test Cases ---\n");
        for (const char* s : validExamples) {
            printf("%-15s ==> %s\n", "\"" + s + "\"", isNumber(s));
        }

        printf("\n--- Invalid Test Cases ---\n");
        for (const char* s : invalidExamples) {
            printf("%-15s ==> %s\n", "\"" + s + "\"", isNumber(s));
        }
        return 0;
}
