...
class ReverseBits32BitInteger {
    ...
    public static int reverseBitsLoop(int n) {
        int result = 0;
        for (int i = 0; i < 32; i++) {
            result = (result << 1) | (n & 1);
            n >>>= 1;
    ...
    public static String toBinary32(int val) {
        return String.format("%32s", Integer.toBinaryString(val)).replace(' ', '0');
    }

    public static void main(String[] args) {
        System.out.println("=== LeetCode 190: Reverse Bits (32-Bit Unsigned Integer) ===");
        ...
        for (int n : testCases) {
            ...
            System.out.printf("Input  : %11d | %s%n", n, toBinary32(n));
            System.out.printf("Loop   : %11d | %s%n", rev1, toBinary32(rev1));
            System.out.printf("D&C    : %11d | %s%n%n", rev2, toBinary32(rev2));
        }
    }
}
