 ...
 */
class ContinuousSubarraySumModuloK {
    ...
    public static void main(String[] args) {
        System.out.println("=== LeetCode 523: Continuous Subarray Sum (Modulo K Map) ===");
        ...
        for (int i = 0; i < testArrays.length; i++) {
            boolean result = checkSubarraySum(testArrays[i], kValues[i]);
            System.out.printf("Array: %-20s | k = %2d ==> Good Subarray: %s%n",
                    java.util.Arrays.toString(testArrays[i]), kValues[i], result);
        }
    }
}
