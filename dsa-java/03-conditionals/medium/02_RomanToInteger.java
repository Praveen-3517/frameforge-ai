...
class RomanToInteger {
    ...
    private static int charValue(char ch) {
        return switch (ch) {
            ...
            case 'D' -> 500;
            case 'M' -> 1000;
            default  -> throw new IllegalArgumentException("Invalid Roman symbol: " + ch);
        };
    }
    ...
    public static int romanToInt(String s) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (i + 1 < n && current < charValue(s.charAt(i + 1))) {
                total -= current;
            } else {
                total += current;
            }
    ...
    public static void main(String[] args) {
        System.out.println("=== LeetCode 13: Roman to Integer (Switch Conditionals) ===");

        String[] testNumerals = { "III", "LVIII", "MCMXCIV", "IX", "XL", "CDXLIV", "MMXXIV" };

        for (String rom : testNumerals) {
            System.out.printf("Roman: %-10s ==> Integer: %d%n", rom, romanToInt(rom));
        }
    }
}
