    ...
    try:
        ...
        transform_result = await loop.run_in_executor(
            ...
            lambda: generate_video_variant_sync(
                ...
                normalize_audio=params.get("normalize_audio", False),
                audio_sample_rate=params.get("audio_sample_rate", 48000),
                strip_metadata=params.get("strip_metadata", True),
                # Deep visual transforms — use params exactly, fallback False (no re-encode by default)
                deep_visual=params.get("deep_visual", False),
                zoom_pct=params.get("zoom_pct", 0.0),
                ...
                add_vignette=params.get("add_vignette", False),
                # Deep audio transforms
                audio_mode=params.get("audio_mode", "max_protection"),
                pitch_shift_semitones=params.get("pitch_shift_semitones", 1.4),
